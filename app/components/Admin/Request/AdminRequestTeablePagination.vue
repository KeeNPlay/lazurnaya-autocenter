...
<template>
  <div v-if="totalPages > 1" class="flex items-center justify-center gap-x-1">
    <button
      type="button"
      class="rounded-md px-3 py-2 text-sm text-gray-300 hover:bg-gray-150 disabled:opacity-30"
    ...
    <template v-for="(page, index) in pages" :key="index">
      <span v-if="page === 'ellipsis'" class="px-2 text-gray-500">...</span>
      <button
        v-else
        type="button"
        class="rounded-md px-3 py-2 text-sm transition-colors duration-150"
        :class="page === currentPage ? 'bg-orange-500 text-white' : 'text-gray-300 hover:bg-gray-150'"
        @click="goTo(page)"
      >
        {{ page }}
      </button>
